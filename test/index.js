...
orchestrator.registerScenario(
  ...
  async (s, t) => {
    ...
    const create_goal = await alice.call('acorn_hc', 'holo_acorn', 'create_goal', {
      goal: {
        content: 'sample content',
        user_hash: alice._instances.acorn_hc.agentAddress,
        unix_timestamp: Date.now(),
    ...
    await s.consistency()
    const time = Date.now()
    const update_goal = await alice.call('acorn_hc', 'holo_acorn', 'update_goal', {
      goal: {
        content: 'sample content2',
        user_hash: alice._instances.acorn_hc.agentAddress,
        unix_timestamp: time,
    ...
    t.equal(first_fetch_goals_result.Ok[0].address, second_fetch_goals_result.Ok[0].address)
  }
)
